...
public final class GlomoRegistrator
{
    ...
    try
    {
      for (int i = 0; (i < paramArrayOfString.length) && (str1 == ""); i++)
      {
        ...
        if ((str2 = paramMIDlet.getAppProperty("MIDlet-" + String.valueOf(i + 1))) == null) {
          str1 = str1 + "+midlet1";
        } else if (str2.trim().compareTo(paramArrayOfString[i].trim()) != 0) {
          str1 = str1 + "+midlet2";
        }
      }
  ...
  public final String getActivationKey(String paramString)
  {
    ...
    return (str + (paramString.compareTo("") == 0 ? "" : new StringBuffer().append(GlomoConfig.distInfo.smsCustomTailSeparator()).append(paramString).toString())).toLowerCase();
  }
    ...
    if ((bool = (!isActivated()) && (paramInt >= 0) && (paramString != null) && (paramString.compareTo("") != 0) ? 1 : 0) != 0)
    {
      ...
      } else if ((paramInt == 5) || (paramInt == 6) || (paramInt == 7) || (paramInt == 8))
      {
        if ((bool = isSubscribed(false))) {
          ...
          {
            paramString = this;
            void tmp100_97 = new Date();
            Date localDate;
            tmp100_97.setTime((localDate = tmp100_97).getTime() + Long.parseLong(paramString.getCountryInfo().durationSubscribe(paramInt)) * 24L * 60L * 60L * 1000L);
            GlomoRMS.ExpirationDate(localDate.getTime());
          }
  ...
  public final boolean isRegistered()
  {
    ...
    try
    {
      bool = Long.parseLong(GlomoUtil.decrypt(GlomoRMS.EnteredKey(), 0)) == GlomoRMS.ActivationKey();
    }
    catch (Exception localException) {}
    return bool;
  }
...
/* Location:              C:\WPrograming\jd-gui-windows-1.4.0\Download_For_Motorola_128x160.jar!\GlomoReg\GlomoRegistrator.class
